...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: {
			...
			hMsgDisplay = CreateWindow("LISTBOX", "",
...
void AddMessage(const char* msg) {
    if (!hMsgDisplay || !msg) return;

    // Add message to LISTBOX
    int index = SendMessage(hMsgDisplay, LB_ADDSTRING, 0, (LPARAM)msg);
    
    if (index != LB_ERR) {
        // Scroll down
        SendMessage(hMsgDisplay, LB_SETTOPINDEX, (WPARAM)(index), 0);
    }
}