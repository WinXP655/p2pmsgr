...
void ShowMainWindow(HINSTANCE hInstance, int nCmdShow) {
    ...
    char title[128];
    sprintf(title, "Chat2 - %s", computer_name);
...
void ShowServerIPMessage() {
    char message[256];
    sprintf(message, "Server IP: %s\nShare with users to connect.\nClose p2pmsgr.exe to stop.", server_ip);
    MessageBox(NULL, message, "Chat2 Server", MB_OK | MB_ICONINFORMATION);
}
...
int ShowConnectionDialog(HWND hwnd, BOOL isIncoming, const char* remoteName, const char* remoteIp) {
    char title[128];
    char header[256];
    char securityWarning[256];
    char roleText[256];

    if (isIncoming) {
        strcpy(title, "P2P Messenger - Incoming Connection");
        strcpy(header, "Chat request received:");
        strcpy(securityWarning, "WARNING: Accepting connections from untrusted computers may expose your system to security risks!");
        sprintf(roleText, "This PC (Server): %s\nRemote PC (User): %s", computer_name, remoteName);
    } else {
        strcpy(title, "P2P Messenger - Outgoing Connection");
        strcpy(header, "You are connecting to:");
        strcpy(securityWarning, "WARNING: Connecting to untrusted servers may compromise your privacy and security!");
        sprintf(roleText, "This PC (User): %s\nRemote PC (Server): %s", computer_name, remoteName);
    }
    ...
    sprintf(message,
        ...
        "\n"
        "%s\n"
        "Do you want to %s this connection?",
        header,
        roleText,
        remoteIp,
        securityWarning,
        isIncoming ? "accept" : "establish");

    int result = MessageBox(hwnd, message, title,
        MB_YESNO | MB_ICONEXCLAMATION | MB_DEFBUTTON2);

    if (result == IDNO) {
        char logMsg[256];
        sprintf(logMsg, "[SECURITY] %s connection to %s (%s) was rejected by user",
            isIncoming ? "Incoming" : "Outgoing",
            remoteName,
            remoteIp);

        if (is_server) LogMessage(logMsg);
        AddMessage(logMsg);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: {
			...
			SendMessage(hEdit, WM_SETFONT, (WPARAM)hFontTahoma, TRUE);
...
void InitializeNetwork(bool server_mode) {
    ...
    if (server_mode) {
		...
		ShowConnectionDialog(hwnd_global, TRUE, peer_name, peer_ip);
		
		char logMsg[256];
		sprintf(logMsg, "[SECURITY] Incoming connection from %s (%s) was accepted by server", peer_name, peer_ip);
		AddMessage(logMsg);
		LogMessage(logMsg);
		...
		// Показываем сообщение о том, что мы ждём подтверждения
        char wait_msg[256];
        sprintf(wait_msg, "[SECURITY] Waiting for server request accept...");
        AddMessage(wait_msg);
        PlayNotificationSoundSystem();
        
        // Показываем диалог подтверждения (пользователь может отказаться)
        if (ShowConnectionDialog(hwnd_global, FALSE, server_ip, server_ip) == IDNO) {
            closesocket(client_socket);
            WSACleanup();
            exit(1);
        }
			...
			switch(error_code) {
				case WSAETIMEDOUT:
					snprintf(error_msg, sizeof(error_msg),
							"Connection to %s timed out.", server_ip);
					break;
				case WSAECONNREFUSED:
					snprintf(error_msg, sizeof(error_msg),
							"Connection to %s was refused.", server_ip);
					break;
				default:
					snprintf(error_msg, sizeof(error_msg),
							"Failed to connect to %s. Error: %d.", server_ip, error_code);
					break;
			}

			MessageBox(NULL, error_msg, "Chat2 - Connection Error", MB_OK | MB_ICONERROR);
		...
		// Уведомляем клиента, что сервер принял запрос
        char accept_msg[256];
        sprintf(accept_msg, "[SECURITY] Server accepted your chat request");
        AddMessage(accept_msg);
        PlayNotificationSoundJoin();
